...
def display_results(ats_results, rag_skills, llm_analysis, resume_text, job_predictor=None):
    ...
    if job_predictor and rag_skills:
        try:
            matched_skills = ats_results.get('similarity_scores', {}).get('matched_skills', [])
            # Combine matched skills with top RAG skills to reduce noise
            top_rag_skills = rag_skills[:80]
            combined_skills = []
            for skill in (matched_skills or []):
                if skill and skill not in combined_skills:
                    combined_skills.append(skill)
            for skill in top_rag_skills:
                if skill and skill not in combined_skills:
                    combined_skills.append(skill)
            skills_text = ' '.join(combined_skills if combined_skills else rag_skills[:80])
